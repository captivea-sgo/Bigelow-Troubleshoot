...
class sale_order(models.Model):
    ...
    def create_or_update_customer(self,vals,is_company=False,parent_id=False,type=False,instance=False):
        ...
        if is_company:
            ...
            partner_vals.update({'property_account_payable_id':instance.shopify_property_account_payable_id.id,'property_account_receivable_id':instance.shopify_property_account_receivable_id.id})
...
#       Guided By : Dhaval Bhalani Sir
    ...
    def import_shopify_orders(self,to_date,from_date,instance=False):
        #self.log.debug = logging.getLogger('shopifysaleorder')
        ...
        for instance in instances:
            ...
            for status in instance.import_shopify_order_status_ids:
                ...
                for order_id in order_ids:
                    ...
                    if not partner:
                        partner = self.env['res.partner'].search([('name','=','MAIN WALKIN')],limit=1)
                    
                    #End Captivea changes
                    if not partner:                    
                    ...
                    for line in result.get('shipping_lines',[]):
                        #self.log.debug.debug('SO 11')
                        tax_ids=self.get_tax_id_ept(instance,line.get('tax_lines'),result.get('taxes_included'))                
                        delivery_method=line.get('title')
                        if delivery_method:
                            #self.log.debug.debug('SO 12')
                ...
                if import_order_ids:
                    #self.log.debug.debug('Calling workflow process')
    ...
    def update_order_status(self,instance):
		...
		# Getting only those picking has state is done, Not updated in shopify and location destination id user is customer, Added By Prakash Makwana Dated on : 26, April 2019 and guided by Dhaval Sir.
                picking_ids = sale_order.picking_ids.filtered(lambda p: p.updated_in_shopify == False and p.state == 'done' and p.location_dest_id.usage == 'customer')
                for picking in picking_ids:
                    ...
                    try:
                        ...
                        if not fulfillment_result:
                            message = "Order(%s) status not updated due to some issue in fulfillment request/response:"%(sale_order.name)
                            if not log:
                                transaction_log_obj.create(
                                    {'message': message,
                                     'mismatch_details': True,
    ...
    def delivery_set(self):
        if self.shopify_order_id:
            raise UserError(_('You are not allow to change manually shipping charge in Shopify order.'))
        else:
    ...
    def unlink(self):
        ...
        for record in self:
            if record.order_id.shopify_order_id:
                msg = _("You can not delete this line because this line is Shopify order line and we need Shopify line id while we are doing update order status")
                raise UserError(msg)
